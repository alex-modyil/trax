...
def append_value(generator, val=None):
  for example in generator:
    if val is not None:
      for key, value in val.items():
        example[key] = np.append(example[key], value, len(np.shape(val))-1)
    yield example


def AppendValue(val=None):  # pylint: disable=invalid-name
  """Appends values provided in 'val`.

  Args:
    val: dict of int to tensors. Specific keys get the tensors specified in
      values appended.
  """
  return lambda g: append_value(g, val)


def truncate_to_len(generator, len_map=None):
  for example in generator:
...
def TruncateToLen(len_map=None):  # pylint: disable=invalid-name
  ...
  """
  return lambda g: truncate_to_len(g, len_map)
